...
# Which classes do you want to detect? Edit this list.
# Examples: "person", "dog", "cat", "bird", "car", "bicycle", "motorbike"
# Leave empty [] to detect ALL classes available in the model.
DETECT_CLASS_NAMES = ["person", "dog", "cat", "car"]
...
LATEST_FRAME_SAVE_MIN_INTERVAL = 1.0
...
# -------------------------
# Model loading (returns mapping for requested classes)
# -------------------------
def load_model_and_class_map(model_path, requested_class_names):
    """
    Load YOLO model and build a mapping {class_name: class_id} for requested_class_names.
    If requested_class_names is empty, return an empty dict meaning "detect all classes".
    """
    model = YOLO(model_path)
    # model.names is a dict: id->name
    ...
    if requested_class_names:
        for cname in requested_class_names:
            if cname in name_to_id:
                class_map[cname] = name_to_id[cname]
            else:
                print(f"[WARN] Class '{cname}' not found in model.names; skipping.")
    else:
        # empty => user wants all classes; return empty mapping to signal "all"
        class_map = {}

    if class_map:
        print(f"[INFO] Model loaded. Will detect classes: {class_map}")
    else:
        print("[INFO] Model loaded. Will detect ALL classes (DETECT_CLASS_NAMES empty).")

    return model, class_map


# -------------------------
# DB (now stores class info + bbox)
# -------------------------
def init_db(path):
...
# Sound player (async) - minimal beep fallback
...
def _play_sound_worker(path=None):
    try:
        ...
        if shutil.which("play"):
            subprocess.Popen(["play", "-n", "synth", "0.5", "sine", "1500"], stdout=subprocess.DEVNULL,
...
def main():
    ...
    if classes_list:
        print("[INFO] Detecting class IDs:", classes_list)
    else:
        print("[INFO] Detecting ALL classes from model.")
    ...
    try:
        while True:
                ...
                for r in results:
                    ...
                    for box, cls_id_raw, track_id in zip(r.boxes.xyxy, r.boxes.cls, r.boxes.id):
                        try:
                            ...
                            cls_id = int(cls_id_raw.item()) if hasattr(cls_id_raw, "item") else int(cls_id_raw)

                        # skip if user requested specific classes and this isn't one of them
            ...
            # Display quickly
            cv2.imshow("Multi-Class Detection (Low Latency)", annotated_frame)
